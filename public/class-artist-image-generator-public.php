...
class Artist_Image_Generator_Public
{
    ...
    // Add function to generate the shortcode
    public function aig_shortcode($atts)
    {
        $atts = shortcode_atts(
            array(
                ...
                'n' => '3',
                'size' => '1024x1024',
                'model' => 'dall-e-2', // New parameter to select the model
                'download' => 'manual' // New parameter to select the download behavior
            ),
            $atts
        );

        // If the model is 'dall-e-3', adjust the value of 'n' to 1
            ...
            <form method="post" class="aig-form" data-action="generate_image" data-n="<?php echo esc_attr($atts['n']); ?>" data-size="<?php echo esc_attr($atts['size']); ?>" data-model="<?php echo esc_attr($atts['model']); ?>" data-download="<?php echo esc_attr($atts['download']); ?>" action="<?php echo admin_url('admin-ajax.php'); ?>">
                ...
                <div class="form-group">
                    <fieldset class="aig-topic-buttons">
                        <legend class="form-label">オプション:</legend>
                        <?php
                    ...
                    <small id="aig_topics_help" class="form-text text-muted">画像生成のために1つ以上のオプションを選択</small>
                </div>
                <hr />
                <div class="form-group">
                    <label for="aig_public_prompt" class="form-label">説明:</label>
                    <textarea name="aig_public_prompt" id="aig_public_prompt" class="form-control" placeholder="画像生成の説明を入力してください（例：'千葉県の海沿いの家'）。"></textarea>
                    <br>
                    <small id="aig_public_prompt_help" class="form-text text-muted">画像の簡単な説明を入力</small>
                </div>
                <hr class="aig-results-separator" style="display:none" />
                <div class="aig-results"></div>
                <hr />
                <button type="submit" class="btn btn-primary">画像生成 / 再試行</button>
            </form>
        </div>
<?php
        return ob_get_clean(); // バッファに保存されたHTMLコンテンツを取得し、それを返す
    }
}
